...
class SoBaikePurifier(BasePurifier):
    ...
    def purify(self, doc: Document) -> Document:
        ...
        selector.xpath("//div[@id='main-content-text']//a[@class='reference_sup']").drop()
        selector.xpath("//div[@id='main-content-text']//span[@class='opt js-edittext']").drop()
        selector.xpath("//a[@class='show-img layoutright']").drop()

        # 标题
        title = selector.xpath("//div[@id='baike-title']/h1/span[@class='title']/text()").get()
        clean_data = f"# {title}\r\n"

        # 所属类别
        classify_key = selector.xpath("//div[@class='entry-classify-down js-classify-list']/span/text()").get()
        classify_val = selector.xpath("//div[@class='classify-txt js-classify-txt short']/text()").get()
        ...
        # 基本信息
        base_info_dict = {}
        base_info_ul_list = selector.xpath("//div[@id='basic-info']/div[@class='card-list-box']/ul")
        ...
        for base_info_ul_item in base_info_ul_list:
            ...
            for li_item in cur_li_list:
                key = li_item.xpath("./div/p[starts-with(@class,'cardlist-name')]/@title").get()
                value = li_item.xpath("./div/p[starts-with(@class,'cardlist-value')]/@title").get()
        ...
        for not_visible_item in not_visible_elements:
            content_label.xpath(".//*[@class='" + str(not_visible_item)[1:] + "']").drop()
